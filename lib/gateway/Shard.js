...
class Shard extends EventEmitter {
        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "CHANNEL_DELETE": {
                if(packet.d.type === 1 || packet.d.type === undefined) {
                    ...
                    if(channel.type === 2) {
                        for (const [key, member] of channel.voiceMembers) {
            ...
            case "READY": {
                ...
                if(this.client.user.bot) {
                    ...
                    for (const guildSettings of packet.d.user_guild_settings) {
                ...
                for (const channel of packet.d.private_channels) {
                ...
                if(packet.d.relationships) {
                    for (const relationship of packet.d.relationships) {
    ...
    * @param {Number} [game.type] The type of game. 0 is default, 1 is streaming (Twitch only)
    ...
    */
    editStatus(status, game) {
        ...
        for (const [key, guild] of this.client.guilds) {
