...
class RequestHandler {
    constructor(client, forceQueueing) {
        ...
        this.userAgent = `DiscordBot (https://github.com/ParadoxalCorp/eris, ${require("../../package.json").version}) (Forked from (https://github.com/abalabahaha/eris))`;
        this.ratelimits = {};
    ...
    globalUnblock() {
        if (this._client.redis && this._client.redis.status === "ready") {
            this._client.redis.del(redisKeys.GLOBAL_RATELIMIT)
                .catch(err => this._client.emit("error", err))
        }
        this.globalBlock = false;
        this._unblockTimeout = null;
    ...
    */
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise(async(resolve, reject) => {
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.headers["retry-after"]) {
                            if(resp.headers["x-ratelimit-global"]) {
                                await this._globalBlock(+resp.headers["retry-after"] || 1).catch(err => this._client.emit("error", err));
            ...
            if(await this._isGloballyBlocked() && auth) {
     ...
     */
    async _isGloballyBlocked () {
        if (this._client.redis && this._client.redis.status === "ready") {
            return !!(await this._client.redis.get(redisKeys.GLOBAL_RATELIMIT)
                .then(res => {
                    if (res && Number(res) < Date.now()) {
                        return false;
                    }
                    if (!this._unblockTimeout) {
                        this._unblockTimeout = setTimeout(this.globalUnblock.bind(this), Date.now() - Number(res));
                    }
                    return true;
                })
                .catch(err => this._client.emit("error", err)));
    ...
    async _globalBlock (retryAfter) {
        if (this._client.redis && this._client.redis.status === "ready") {
            return !!(await this._client.redis.set(redisKeys.GLOBAL_RATELIMIT, Date.now() + retryAfter)
                .catch(err => this._client.emit("error", err)));
        }
        return this.globalBlock;
    }
