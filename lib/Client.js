...
class Client extends EventEmitter {
    ...
    * @param {Object} [options] Eris options (all options are optional)
    * @param {Boolean} [options.autoreconnect=true] Have Eris autoreconnect when connection is lost, defaults to `true`
    * @param {Boolean} [options.compress=false] Whether to request WebSocket data to be compressed or not, defaults to `false`
    * @param {Number} [options.connectionTimeout=30000] How long in milliseconds to wait for the connection to handshake with the server, defaults to `30000`
    * @param {Object} [options.disableEvents] If `disableEvents[eventName]` is true, the WS event will not be processed. This can cause significant performance increase on large bots. [A full list of the WS event names can be found on the docs reference page](/Eris/docs/reference#ws-event-names)
    * @param {Boolean} [options.disableEveryone=true] When true, filter out `everyone`/`here` by default in createMessage/editMessage, defaults to `true`
    * @param {Number} [options.firstShardID=0] The ID of the first shard to run for this client
    * @param {Boolean} [options.getAllUsers=false] Get all the users in every guild. Ready time will be severely delayed
    * @param {Number} [options.guildCreateTimeout=2000] How long in milliseconds to wait for a GUILD_CREATE before "ready" is fired. Increase this value if you notice missing guilds, defaults to `2000`
    * @param {Number} [options.largeThreshold=250] The maximum number of offline users per guild during initial guild data transmission, defaults to `250`
    * @param {Number} [options.latencyThreshold=30000] The average request latency at which Eris will start emitting latency errors
    * @param {Number} [options.lastShardID=options.maxShards - 1] The ID of the last shard to run for this client
    * @param {Number | String} [options.maxShards=1] The total number of shards you want to run. If "auto" Eris will use Discord's recommended shard count., defaults to `1`
    * @param {Number} [options.messageLimit=100] The maximum size of a channel message cache, defaults to `100`
    * @param {Boolean} [options.opusOnly=false] Whether to suppress the node-opus not found error or not, defaults to `false`
    * @param {Number} [options.ratelimiterOffset=0] A number of milliseconds to offset the ratelimit timing calculations by
    * @param {Number} [options.requestTimeout=15000] A number of milliseconds before requests are considered timed out, defaults to `15000`
    * @param {Boolean} [options.restMode=false] Whether to enable getting objects over REST. This should only be enabled if you are not connecting to the gateway. Bot tokens must be prefixed manually in REST mode, defaults to `false`
    * @param {Boolean} [options.seedVoiceConnections=false] Whether to populate bot.voiceConnections with existing connections the bot account has during startup. Note that this will disconnect connections from other bot sessions, defaults to `false`
    * @param {String} [options.defaultImageFormat="webp"] The default format to provide user avatars, guild icons, and group icons in. Can be "jpg", "png", "gif", or "webp", defaults to `webp`
    * @param {Number} [options.defaultImageSize=128] The default size to return user avatars, guild icons, and group icons as. Can be any power of two between 16 and 2048, defaults to `128`
    * @param {Object} [options.ws] An object of WebSocket options to pass to the shard WebSocket constructors
    * @param {Object} [options.agent] A HTTP Agent used to proxy requests
    * @param {RedisOptions} [options.redis] Options to pass directly to the `ioredis` constructor, if passed, eris will connect to redis and use it to synchronize global ratelimits across all clusters
    */
    constructor(token, options) {
        ...
        if(this.options.lastShardID === undefined && this.options.maxShards !== "auto" && typeof this.options.maxShards === "number") {
        ...
        /** @type {IOredis.Redis} */
        this.redis = options.redis ? new IOredis(null, null, { lazyConnect: true, ...options.redis }) : null;
    ...
    /**
    * Tells all shards (and redis if options.redis was passed) to connect
    * @param {Boolean} [connectToRedis] Whether to connect to redis (if options.redis is passed) as well, defaults to `true`
    * @returns {Promise} Resolves when all shards are initialized
    */
    async connect(connectToRedis = true) {
        try {
            if (this.redis && connectToRedis && !["ready", "connect", "connecting"].includes(this.redis.status)) {
                await this.redis.connect()
                    .catch(err => {
                        this.emit("error", err);
                        this._retryConnectionToRedis();
                    });
            }
    ...
    * @param {Object} [options] The properties the channel should have
    ...
    * @param {String?} [options.parentID] The ID of the parent channel category for this channel
    ...
    * @param {String} [options.icon] The icon of the channel as a base64 data URI (group channels only). Note: base64 strings alone are not base64 data URI strings
    * @param {String} [options.ownerID] The ID of the channel owner (group channels only)
    ...
    * @param {Object | Object[]} [options.file] A file object (or an Array of them)
    * @param {Buffer} options.file.file A buffer containing file data
    * @param {String} options.file.name What to name the file
    * @param {Object[]} [options.embeds] An array of Discord embeds
    ...
    * @param {Boolean} [options.disableEveryone] Whether to filter @everyone/@here or not (overrides default)
    * @returns {Promise}
    */
    ...
    * @param {Number} [options.permissions] The role permissions number
    ...
    * @param {Object} options The properties to edit
    * @param {String} [options.expireBehavior] What to do when a user's subscription runs out
    * @param {String} [options.expireGracePeriod] How long before the integration's role is removed from an unsubscribed user
    * @param {String} [options.enableEmoticons] Whether to enable integration emoticons or not
    * @returns {Promise}
    */
    ...
    * @param {String} guildID The ID of the guild
    * @param {Object} options The properties to edit
    * @param {String} [options.name] The ID of the guild
    ...
    * @param {Object} options The properties to edit
    * @param {String[]} [options.roles] The array of role IDs the member should have
    ...
    * @param {String} [options.channelID] The ID of the voice channel to move the member to (must be in voice)
    * @param {String} [reason] The reason to be displayed in audit logs
    * @returns {Promise}
    */
    ...
    */
    ...
    /**
     * Disconnects the given user from the voice channel they are in
     * @param {String} guildID The ID of the guild
     * @param {String} userID The ID of the user
     * @returns {Promise}
     */
    disconnectGuildMember (guildID, userID) {
