...
class Channel extends Base {
    ...
    isType(type) {
        ...
        for (const key in constants.channelTypes) {
            if (type === key || type === constants.channelTypes[key].API_TYPE || type === constants.channelTypes[key].FRIENDLY_TYPE) {
                return true;
            }
        }
        return false;
    ...
    get friendlyType() {
        for (const key in constants.channelTypes) {
            if (constants.channelTypes[key].API_TYPE === this.type) {
                return constants.channelTypes[key].FRIENDLY_TYPE;
            }
        }
